...
def generate():
  # Use request.get_json() to get the JSON payload from the request body
    ...
    prompt = f"Using the following trip data, generate 2 different day by day travel plans and put the location plans in json (note : $ = shoestring, $$ = budget, $$$ = comfortable, $$$$ = slurge, $$$$$ = luxury): ```json\n{json.dumps(payload, indent=2)}\n```"
    print(prompt)
    try:
